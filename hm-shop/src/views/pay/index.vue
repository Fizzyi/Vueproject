<template>
  <div class="pay">
    ...
    <div class="pay-list" v-if="order.goodsList">
      ...
      <!-- 买家留言 -->
      <div class="buytips">
        <textarea placeholder="选填：买家留言（50字内）" name="111" id="" cols="30" rows="10"></textarea>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async getAddressList () {
      ...
      console.log('地址信息为' + list)
      this.addressList = list
    },
    async getOrderList () {
      if (this.mode === 'cart') {
        console.log(await checkOrder(this.mode, { cartIds: this.cartIds }))
        const { data: { order, personal } } = await checkOrder(this.mode, { cartIds: this.cartIds })
        this.order = order
        this.personal = personal
        console.log('order' + order.orderTotalPrice)
      }
      if (this.mode === 'buyNow') {
