    ...
    <div class="container">
      ...
      <div class="form">
        ...
        <div class="form-item">
          ...
          <button @click="getCode">{{ second === totalSeconde ? '获取验证码':second + '秒后重新发送' }} </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getCode () {
      ...
      if (!this.timer && this.second === this.totalSeconde) {
        // 发送请求，获取短信验证码
        ...
        this.timer = setInterval(() => {
          this.second--
          if (this.second < 1) {
            clearInterval(this.timer) // 取消定时任务
            this.timer = null
  ...
  destroyed () {
    clearInterval(this.timer) // 离开页面时销毁定时器
  }
}
</script>
